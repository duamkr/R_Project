...
# 두 변수의 등분산성 확인(
# Robust Brown-Forsythe 방법은 중앙값(median)을 기준으로 동질성 검정 수행.(분포상 평균값이 집단을 대표하지 못한다고 판단되는 경우)

levene.test(kobis_avengers$people , kobis_avengers$daet)

# p-value > 0.05 이므로 두 집단의 분산이 다르므로 대립가설을 지지합니다.
# 따라서 네이버 일자별 네이버 댓글수는 전체 영화관람객수와 연관이 없다.
```
